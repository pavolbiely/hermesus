...
const previewPath = computed(() => props.preview?.relativePath || props.preview?.path || props.preview?.requestedPath || props.requestedPath || 'File preview')
const meta = computed(() => {
  if (!props.preview) return null
  return `${props.preview.mediaType} · ${formatBytes(props.preview.size)}${props.preview.truncated ? ' · truncated' : ''}`
})
const isCodeHighlightPending = computed(() => Boolean(props.preview && isCodePreview.value && !highlightedCodeHtml.value))
const showLoadingState = computed(() => props.loading || isCodeHighlightPending.value)
    ...
    <template #content>
      <div class="relative flex h-[85vh] max-h-[calc(100dvh-2rem)] flex-col overflow-hidden rounded-lg bg-default shadow-xl ring ring-default">
        ...
        <div class="min-h-[60vh] flex-1 overflow-auto p-4">
          <div v-if="showLoadingState" class="flex min-h-[60vh] items-center justify-center gap-2 text-sm text-muted">
          ...
          <div v-else-if="preview && isMarkdown" ref="markdownPreviewRoot" class="min-h-[60vh]">
            <Comark
          ...
          <div
            ...
            class="chat-file-preview-code min-h-[60vh] *:first:mt-0 *:last:mb-0"
